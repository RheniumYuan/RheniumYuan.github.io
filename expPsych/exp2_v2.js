var jsPsych = initJsPsych({
  on_finish: function() {
      ...
      console.log(id);
      //保存文件名
      filename = `data_${id}_${age}_${sex}_exp2`;
...
//初始化实验数据，方向，提示，刺激情绪，是否反应，方向判断
var expData = [['prompt','dir','valence','yesno','direction']];
var tempData;
var directionSeries = [];
...
var negativePrompt = {
  type: jsPsychHtmlKeyboardResponse,
  stimulus: '<p style="font-size:48px; color:white;">请忽略之后出现的消极情绪的图片</p>',
  choices: 'NO_KEYS',
  trial_duration: 1000,
  on_finish: function(){
      tempData = [];
...
var positivePrompt = {
  type: jsPsychHtmlKeyboardResponse,
  stimulus: '<p style="font-size:48px; color:white;">请忽略之后出现的积极情绪的图片</p>',
  choices: 'NO_KEYS',
  trial_duration: 1000,
  on_finish: function(){
      tempData = [];
...
{
  orderTrial.push(1);//正性，无提示
  orderTrial.push(2);//正性，正性提示
  orderTrial.push(3);//正性，负性提示
  orderTrial.push(4);//负性，无提示
  orderTrial.push(5);//负性，正性提示
  orderTrial.push(6);//负性，负性提示
}
...
for (var k=0;k<orderTrial.length;k++)
{
  timeline.push(space);
  if (orderTrial[k]==1){
    timeline.push(noPrompt);
    timeline.push(fixation);
    timeline.push(trialPos);
  }
  if (orderTrial[k]==2){
    timeline.push(positivePrompt);
    timeline.push(fixation);
    timeline.push(trialPos);
  }
  if (orderTrial[k]==3){
    timeline.push(negativePrompt);
    timeline.push(fixation);
    timeline.push(trialPos);
  }
  if (orderTrial[k]==4){
